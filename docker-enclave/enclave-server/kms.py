...
class NitroKms():
    ...
    def kms_list_aliases(self):
        amz_target = 'TrentService.ListAliases'
        request_parameters = json.dumps({
            "Limit": 100
        })
        return self._kms_call(amz_target, request_parameters)
    ...
    def kms_create_key(self):
        amz_target = 'TrentService.CreateKey'
        request_parameters = json.dumps({
            "BypassPolicyLockoutSafetyCheck": False,
            "CustomerMasterKeySpec": "SYMMETRIC_DEFAULT",
            "Description": "maybe add the attestation doc? max 8192 char",
            "Origin": "AWS_KMS",
            "Policy": json.dumps({"Id": "key-consolepolicy-3","Version": "2012-10-17","Statement": [{"Sid": "Enable IAM User Permissions","Effect": "Allow","Principal": {"AWS": "arn:aws:iam::864779222181:root"},"Action": "kms:*","Resource": "*"},{"Sid": "Allow access for Key Administrators","Effect": "Allow","Principal": {"AWS": "arn:aws:iam::864779222181:role/admin"},"Action": ["kms:Create*","kms:Describe*","kms:Enable*","kms:List*","kms:Put*","kms:Update*","kms:Revoke*","kms:Disable*","kms:Get*","kms:Delete*","kms:TagResource","kms:UntagResource","kms:ScheduleKeyDeletion","kms:CancelKeyDeletion"],"Resource": "*"},{"Sid": "Allow use of the key","Effect": "Allow","Principal": {"AWS": "arn:aws:iam::864779222181:role/admin"},"Action": ["kms:Encrypt","kms:Decrypt","kms:ReEncrypt*","kms:GenerateDataKey*","kms:DescribeKey"],"Resource": "*"},{"Sid": "Allow attachment of persistent resources","Effect": "Allow","Principal": {"AWS": "arn:aws:iam::864779222181:role/admin"},"Action": ["kms:CreateGrant","kms:ListGrants","kms:RevokeGrant"],"Resource": "*","Condition": {"Bool": {"kms:GrantIsForAWSResource": "true"}}}]}),
            #"Policy": json.dumps({     "Id": "key-consolepolicy-3",     "Version": "2012-10-17",     "Statement": [         {             "Sid": "Enable IAM User Permissions",             "Effect": "Allow",             "Principal": {                 "AWS": "arn:aws:iam::722090610812:root"             },             "Action": "kms:*",             "Resource": "*"         },         {             "Sid": "Allow access for Key Administrators",             "Effect": "Allow",             "Principal": {                 "AWS": "arn:aws:iam::722090610812:role/fullaccess1"             },             "Action": [                 "kms:Create*",                 "kms:Describe*",                 "kms:Enable*",                 "kms:List*",                 "kms:Put*",                 "kms:Update*",                 "kms:Revoke*",                 "kms:Disable*",                 "kms:Get*",                 "kms:Delete*",                 "kms:TagResource",                 "kms:UntagResource",                 "kms:ScheduleKeyDeletion",                 "kms:CancelKeyDeletion"             ],             "Resource": "*"         },         {             "Sid": "Allow use of the key",             "Effect": "Allow",             "Principal": {                 "AWS": "arn:aws:iam::722090610812:role/fullaccess1"             },             "Action": [                 "kms:Encrypt",                 "kms:Decrypt",                 "kms:ReEncrypt*",                 "kms:GenerateDataKey*",                 "kms:DescribeKey"             ],             "Resource": "*"         },         {             "Sid": "Allow attachment of persistent resources",             "Effect": "Allow",             "Principal": {                 "AWS": "arn:aws:iam::722090610812:role/fullaccess1"             },             "Action": [                 "kms:CreateGrant",                 "kms:ListGrants",                 "kms:RevokeGrant"             ],             "Resource": "*",             "Condition": {                 "Bool": {                     "kms:GrantIsForAWSResource": "true"                 }             }         }     ] }),
            "Tags": []
        })
        return self._kms_call(amz_target, request_parameters)

    def kms_create_alias(self, keyid, alias_name):
        amz_target = 'TrentService.CreateAlias'
        request_parameters = json.dumps({
    ...
    def kms_delete_key(self, keyid):
        amz_target = 'TrentService.ScheduleKeyDeletion'
        request_parameters = json.dumps({
            "KeyId": keyid,
    ...
    def kms_decrypt(self, kms_key_id, ciphertext_blob):
        ...
        return kms_response, plaintext_bytes
    ...
    def _get_attestation_doc_b64(self):
        ...
        libnsm_att_doc_cose_signed = libnsm.nsm_get_attestation_doc( # pylint:disable=c-extension-no-member
            ...
            bytes.fromhex("00"*1), 1 #FIXME: replace dummy nonce_data
        )
